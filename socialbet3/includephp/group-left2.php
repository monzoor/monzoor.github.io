<div class="left-container-wrapper group-left t-d-view">
	<div class="top-avatar pos-rel">
 		<img src="images/test-img-1.jpg" class="circle">
		<div class="new-avatar pos-abs abs-bottom-zero">
 			<a href="">
				<span class="white">Загрузить фотографию</span>
 				<span class="icon-photo white"></span>
 			</a>
 		</div>
 	</div>
	<div class="col-wp">
 		<h1>ФК «Зенит»</h1>
 		<p class="mg-top font14">
 			<span class="brand-blue">Зенит–</span> главный российский фестиваль современных технологий, науки и искусства, ключевое событие для прогрессивных, образованных, увлеченных людей, место встречи уникального сообщества, которое создаёт новый мир.
 		</p>
 		<a href=""><p class="selected b mg-top">Управление группой</p></a>
 		<p data-modal="modal-16" class="md-trigger b mg-top">Пригласить участников</p>
 		<p data-modal="modal-17" class="md-trigger b mg-top">Назначить руководителя</p>

		<h1 class="fl mg20">Подписчики</h1>
		<p class="fr brand-blue font14 b count">26 700 подписчиков</p>
		<div class="clear"></div>
		<div class="col-wop mg-top">
			<div class="img-holder small fl">
		 		<img src="images/test-img-1.jpg" class="circle">
		 	</div>
		 	<div class="img-holder small fl">
		 		<img src="images/test-img-1.jpg" class="circle">
		 	</div>
		 	<div class="img-holder small fl">
		 		<img src="images/test-img-1.jpg" class="circle">
		 	</div>
		 	<div class="img-holder small fl">
		 		<img src="images/test-img-1.jpg" class="circle">
		 	</div>
		 	<div class="img-holder small fl">
		 		<img src="images/test-img-1.jpg" class="circle">
		 	</div>
		 	<div class="img-holder small fl">
		 		<img src="images/test-img-1.jpg" class="circle">
		 	</div>
		 	<div class="img-holder small fl">
		 		<img src="images/test-img-1.jpg" class="circle">
		 	</div>
		 	<div class="img-holder small fl">
		 		<img src="images/test-img-1.jpg" class="circle">
		 	</div>
		 	<div class="img-holder small fl">
		 		<img src="images/test-img-1.jpg" class="circle">
		 	</div>
		 	<div class="img-holder small fl">
		 		<img src="images/test-img-1.jpg" class="circle">
		 	</div>
		</div>
		<h1 class="fl mg-top">Обсуждения</h1>
		<p class="fr brand-blue font14 b count">769 тем</p>
		<div class="clear"></div>
		<div class="col-wop mg-top quote">
			<div class="quote-sign fl"><img src="images/quote.jpg" alt=""></div>
			<div class="text-box fl">
				<p class="font14">Фильмы для серьёзных обзоров</p>
				<p class="placeholder-color font12 fl mg-top">Сегодня в 22:14</p>
				<span class="icon-message brand-blue mg-top fl"></span><span class="dark-gray b font14 mg-top fl">15 967</span>
			</div>
		</div>
		<div class="col-wop mg-top quote">
			<div class="quote-sign fl"><img src="images/quote.jpg" alt=""></div>
			<div class="text-box fl">
				<p class="font14">Встреча клуба в Санкт-Петербурге 15 июля </p>
				<p class="placeholder-color font12 fl mg-top">Сегодня в 22:14</p>
				<span class="icon-message brand-blue mg-top fl"></span><span class="dark-gray b font14 mg-top fl">15 967</span>
			</div>
		</div>
		<div class="col-wop mg-top quote">
			<div class="quote-sign fl"><img src="images/quote.jpg" alt=""></div>
			<div class="text-box fl">
				<p class="font14">Фильмы для серьёзных обзоров</p>
				<p class="placeholder-color font12 fl mg-top">Сегодня в 22:14</p>
				<span class="icon-message brand-blue mg-top fl"></span><span class="dark-gray b font14 mg-top fl">15 967</span>
			</div>
		</div>
		<h1 class="fl mg-top">Контакты</h1>
		<div class="clear"></div>
		<div class="image-info-box mg-top">
			<div class="img-holder">
		 		<img src="images/test-img-1.jpg" class="circle">
		 	</div>
		 	<div class="info-holder">
		 		<p>Константин Константиновский</p>
		 		<p class="sub-info">Сегодня в 22:14</p>
		 	</div>
		</div>
		<div class="image-info-box mg-top">
			<div class="img-holder">
		 		<img src="images/test-img-1.jpg" class="circle">
		 	</div>
		 	<div class="info-holder">
		 		<p>Константин Константиновский</p>
		 		<p class="sub-info">Сегодня в 22:14</p>
		 	</div>
		</div>
	</div>
</div>


				