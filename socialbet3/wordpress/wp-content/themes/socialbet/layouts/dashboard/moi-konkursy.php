...
?>
	...
	<h1 class="fl wd-auto"><?php printf( __('Мои конкурсы <span>%d</span>', 'socialbet'), count($competitions) ) ; ?></h1>
		...
		<div class="filter-box mg-bottom">
			<ul>
				<li<?php if ( ! isset($_GET['status']) ) echo ' class="selected"'; ?>>
					<a href="<?php echo get_socbet_user_dashboard_url( $profile_user->ID, _transliteration_process( esc_html( urldecode('мои-конкурсы') ), '?', 'ru' ) ); ?>">
				...
				<li<?php if ( isset($_GET['status']) && $_GET['status'] == 'aktivnyye' ) echo ' class="selected"'; ?>>
					<a href="<?php echo add_query_arg( 'status', 'aktivnyye', get_socbet_user_dashboard_url( $profile_user->ID, _transliteration_process( esc_html( urldecode('мои-конкурсы') ), '?', 'ru' ) ) ); ?>">
				...
				<li<?php if ( isset($_GET['status']) && $_GET['status'] == 'zavershennyye' ) echo ' class="selected"'; ?>>
					<a href="<?php echo add_query_arg( 'status', 'zavershennyye', get_socbet_user_dashboard_url( $profile_user->ID, _transliteration_process( esc_html( urldecode('мои-конкурсы') ), '?', 'ru' ) ) ); ?>">
		...
		?>
			<ul>
				<?php
					...
					global $post;
				?>
				<li id="competition-<?php the_ID(); ?>" <?php post_class('use-shadow'); ?>>
					...
					<div class="col-wp light-gray-bg">
						<p align="center" class="placeholder-color"><a href="#"><u><?php esc_html_e('Не участвовать в конкурсе', 'socialbet'); ?></u></a></p>
					</div>
