...
if ( ! defined('SBSOON_VERSION') ) 
	define( 'SBSOON_VERSION', '1.2' );

if( ! class_exists('Socbet_Admin_Listing') ) {

class Socbet_Admin_Listing {
	...
	public static function init() {
		...
		if ( ! $instance )
			$instance = new Socbet_Admin_Listing;

		return $instance;
	}


	public function Socbet_Admin_Listing() {
		$this->admin_includes();
		$this->admin_init();
	}
	...
	function admin_includes() {
		require_once( get_template_directory() .'/inc/lists/admin/admin-functions.php' );
	}
    ...
    public function admin_enqueue_scripts( $hook ) {
        ...
        wp_enqueue_style( 'socbet_admin',get_template_directory_uri() . '/inc/lists/admin/css/settings.css', '', SBSOON_VERSION );
    }
    ...
    public function email_listing() {
		...
		if ( isset( $_GET['action'] ) ) {
			...
			if ( $_GET['action'] == 'delete' ) {
				...
				// check for security
		    	if ( empty( $_GET['_wpnonce'] ) || ! wp_verify_nonce( $_GET['_wpnonce'], 'socbet-invite-email' ) ) {
					wp_safe_redirect( admin_url( 'admin.php?page=socbet-email-listing' ) );
					exit;
		    	}

		    	$emid = isset( $_GET['eid'] ) ? absint( $_GET['eid'] ) : '0';
		    	$table = $wpdb->prefix . "socbet_subscribes";
		    	$row = $wpdb->get_row( $wpdb->prepare( "SELECT email FROM $table WHERE id = %d", esc_sql($emid) ) );
		    	...
		    	$sent_invite = socbet_sent_invitation_email( sanitize_email($row->email) );

		    	if ( $sent_invite != '1' ) {
					wp_safe_redirect( admin_url( 'admin.php?page=socbet-email-listing&error_invite=1' ) );
					exit;
		    	} else {
					wp_safe_redirect( admin_url( 'admin.php?page=socbet-email-listing&invited=1' ) );
					exit;
		    	}
			} 
		}
		...
		<div id="socbet-body">
			<?php
				...
				if ( isset( $_GET['invited'] ) && $_GET['invited'] == '1' ) {
					echo '<div id="message" class="updated fade"><p><strong>' . esc_html__('Invitation is successfully sent!', 'socialbet') . '</strong></p></div>';
				}

				if ( isset( $_GET['error_invite'] ) && $_GET['error_invite'] == '1' ) {
					echo '<div id="message" class="error fade"><p><strong>' . esc_html__('Cannot sent the invitation! Please try again later.', 'socialbet') . '</strong></p></div>';
				}
			...
			<table class="wp-list-table widefat fixed">
				<thead>
					<tr>
						...
						<th scope="col" class="manage-column"><?php print esc_html__('Invite', 'socialbet'); ?></th>
				...
				<tbody>
					...
					<tr>
						...
						<td><?php echo date_i18n('dS F Y', strtotime($email->registered)); ?></td>
						...
						<td>
							...
							if ( $email->email_status == '0' ) {
								echo '<span class="info-box danger">&mdash;</span>';
							} else {
								if ( email_is_already_invited($email->email) ) {
									echo '<span class="info-box success">' . esc_html__('Invited', 'socialbet') . '</span>';
								} else if ( email_exists( sanitize_email($email->email) ) ) {
									echo '<span class="info-box success">' . esc_html__('Registered', 'socialbet') . '</span>';
								} else {
									?>
									<a class="button-primary" href="<?php echo wp_nonce_url( admin_url("admin.php?page=socbet-email-listing&eid={$email->id}&action=invite"), 'socbet-invite-email', '_wpnonce'); ?>"><?php print __( 'Invite', 'socialbet' ); ?></a>
									<?php
								}
							}
...
add_action( 'init', array('Socbet_Admin_Listing','init') );
